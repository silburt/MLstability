...
int main(int argc, char* argv[]){
    ...
    // Planet 1
    {
        double a=a1, m=m1, inc=reb_random_normal(0.00001);
    ...
    //Planet 2
    {
        double a=a2, m=m2, inc=reb_random_normal(0.00001);
        struct reb_particle p = {0};
        p = reb_tools_orbit_to_particle(r->G, star, m, a, 0, inc, 0, 0, reb_random_uniform(0,2.*M_PI));
        p.r = 0.000467;
        p.hash = r->N;
    ...
    dispersal_time = mig_time/2;                                            //1e4 orbital periods of inner planet
    dispersal_rate = pow(5e7/mig_rate + 1, 1./(dispersal_time/r->dt - 1));  //rate of disk dispersal
    dispersal_fac = 1;
    double tmax = mig_time + dispersal_time*1.25;
